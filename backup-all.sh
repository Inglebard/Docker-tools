...
## "10" ID
## "11" ID_NAME
BACKUP_CONTAINER_FILENAME="01"

FILENAME_NAME_MASK="01"
FILENAME_ID_MASK="10"
...
do
  ...
  CONTAINER_NAME=$(docker ps -a --filter "id=$CONTAINER_ID" --format "{{.Names}}" -q)
  MOUNTS=$(docker inspect --format='{{json .Mounts}}' $CONTAINER_ID)

  CONTAINER_FILENAME=""
  if [ "$(($FILENAME_ID_MASK & $BACKUP_CONTAINER_FILENAME))" -eq "$FILENAME_ID_MASK" ]; then
    CONTAINER_FILENAME="${CONTAINER_ID}_"
  fi
  if [ "$(($FILENAME_NAME_MASK & $BACKUP_CONTAINER_FILENAME))" -eq "$FILENAME_NAME_MASK" ]; then
    CONTAINER_FILENAME="${CONTAINER_FILENAME}${CONTAINER_NAME}_"
  fi
    ...
    then
      ...
      do
        ...
        then
          ...
          then
            FILENAME="${CONTAINER_FILENAME}${volume_name}_${DATE}.tar.gz"
        ...
        then
          ...
          then
            FILENAME="${CONTAINER_FILENAME}${bind_name}_${DATE}.tar.gz"
...
do
  ...
  then
    FILENAME=""
    if [ "$BACKUP_CONTAINER_FILENAME" -eq "0" ]
    then
      FILENAME="${i}_${DATE}.tar.gz"
    else
